...
public class RunLengthEncoding implements Iterable {
  ...
  public PixImage toPixImage() {
    ...
    int x = 0;
    int y = 0;
    for(RunIterator it = this.iterator(); it.hasNext(); ) {
      int[] thisRun = it.next();
      for(int i = 0; i < thisRun[0]; i++) {
        image.setPixel(x, y, (short)thisRun[1], (short)thisRun[2], (short)thisRun[3]);
        x++;
        if(x == width) {
          x = 0;
          y++;
  ...
  public String toString() {
    // Replace the following line with your solution.
    String s = "";
    for(RunIterator it = iterator(); it.hasNext(); ) {
      int[] run = it.next();
      s += run[1] + "," + run[0] + " * ";
    }
    return s;
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    for(int j = 0; j < image.getHeight(); j++) {
      for(int i = 0; i < image.getWidth(); i++) {
        corr = image.getRed(i, j);
        if(corr == prev) {
          count++;
          flag = 1;
        } else {
          Run run = new Run(prev, prev, prev, count);
          head.prev.next = run;
          head.prev.next.next = head;
          head.prev.next.prev = head.prev;
          head.prev = head.prev.next;
  ...
  public void check() {
    ...
    for(RunIterator it = iterator(); it.hasNext(); ) {
      int[] corr = it.next();
      if(prev != null && prev[1] == corr[1]) {
        System.out.println("Error: Consecutive runs have the same RGB intensities!");
        return;
      }
      if(corr[0] < 1) {
        System.out.println("Error: Run length less than 1");
    ...
    if(sum != this.height*this.width) {
      System.out.println("Error: The sum is wrong");
    }
  }
  ...
  public void setPixel(int x, int y, short red, short green, short blue) {
    ...
    //   at the end.
    int num = y*width+x+1;
    Run pointer = head.next;
    int count = 0;
    Run p = null;
    while(pointer != head) {
      count += pointer.getNum();
      if (count >= num) {
        p = pointer;
        break;
      }
    ...
    if (p.getRun()[1] != red) {
      int runNum = p.getNum();
      int last = count-num;
      int first = runNum - last - 1;
      Run r1, r2, r3;
      r1 = new Run(p.getRun()[1], p.getRun()[1], p.getRun()[1], first);
      r2 = new Run(red, green, blue, 1);
      r3 = new Run(p.getRun()[1], p.getRun()[1], p.getRun()[1], last);
      if (first != 0) {
        p.prev.next = r1;
        r1.prev = p.prev;
        r1.next = r2;
        ...
        if (last != 0) {
          ...
          if (red == p.next.getRun()[1]) {
            p.next.setNum(p.next.getRun()[0]+1);
            r1.next = p.next;
            p.next.prev = r1;
          } else {
            r2.next = p.next;
        ...
        if (red == p.prev.getRun()[1]) {
          p.prev.setNum(p.prev.getRun()[0]+1);
          if (last == 0) {
            p.prev.next = p.next;
            p.next.prev = p.prev;
          } else {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Testing setPixel() on a 3x3 encoding.0");
